...
const fmtKm = (km) =>
  (km ?? 0).toLocaleString(undefined, { maximumFractionDigits: 1 });
...
function computeWrapped(items = []) {
  ...
  const runs = items
    ...
    .map(i => {
      ...
      const paceSPerKm = movingS > 0 && distKm > 0
        ? movingS / distKm
        : paceFromAvgSpeed(i.average_speed);
  ...
  const summariseYear = (year) => {
    ...
    const biggestMonth = monthly.reduce(
      (best, cur) => (cur.km > best.km ? cur : best),
      { km: 0, month: null, runs: 0 }
    );

    const mostConsistentMonth = monthly.reduce(
      (best, cur) => (cur.runs > best.runs ? cur : best),
      { runs: 0, month: null, km: 0 }
    );

    // longest day streak
    const daySet = new Set(ys.map(r => r.dayKey).filter(Boolean));
  ...
  const yoy = {
    ...
    avgPaceDeltaSPerKm: (current.avgPaceSPerKm && last.avgPaceSPerKm)
      ? current.avgPaceSPerKm - last.avgPaceSPerKm
      : null,
    avgPaceDeltaPct: (current.avgPaceSPerKm && last.avgPaceSPerKm)
      ? ((current.avgPaceSPerKm - last.avgPaceSPerKm)/last.avgPaceSPerKm)*100 : null,
  };
...
const styles = {
  page: {
    height: "100%",              // fills app panel
    width: "100%",
    overflowY: "auto",           // enables scrolling inside fixed layout
    overflowX: "hidden",
      ...
      <div style={styles.wrap}>
        ...
        <div style={styles.grid4}>
          <StatCard
            ...
            deltaText={`${yoy.distanceDeltaKm >= 0 ? "+" : ""}${fmtKm(yoy.distanceDeltaKm)} km (${fmtPct(yoy.distanceDeltaPct)})`}
          ...
          <StatCard
            ...
            deltaText={`${yoy.timeDeltaSeconds >= 0 ? "+" : ""}${fmtTimeHrs(yoy.timeDeltaSeconds)} (${fmtPct(yoy.timeDeltaPct)})`}
          ...
          <StatCard
            ...
            deltaText={`${yoy.avgRunKmDelta >= 0 ? "+" : ""}${fmtKm(yoy.avgRunKmDelta)} km (${fmtPct(yoy.avgRunKmDeltaPct)})`}
        ...
        <div style={styles.grid2}>
          <StatCard
            ...
            deltaText={
              yoy.avgPaceDeltaSPerKm != null
                ? `${yoy.avgPaceDeltaSPerKm <= 0 ? "faster" : "slower"} by ${fmtPace(Math.abs(yoy.avgPaceDeltaSPerKm))} (${fmtPct(yoy.avgPaceDeltaPct)})`
                : null
            }
          ...
          <StatCard
            ...
            deltaText={`${(current.runsPerWeekAvg - last.runsPerWeekAvg) >= 0 ? "+" : ""}${(current.runsPerWeekAvg - last.runsPerWeekAvg).toFixed(1)} per week`}
        ...
        <Section title="Consistency and habits">
          <div style={styles.grid4}>
            ...
            <StatCard
              ...
              sub={
                last.mostConsistentMonth.month != null
                  ? `${monthLabel(last.mostConsistentMonth.month)} last year`
                  : null
              }
            />
          </div>
        ...
        <Section
          ...
          right={
            <div style={{ fontSize: 12, color: "rgba(229,231,235,0.6)" }}>
          ...
          <div style={{ height: 320 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyChart} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey={String(lastYear)} radius={[8, 8, 0, 0]} opacity={0.5} />
                <Bar dataKey={String(currentYear)} radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthlyChart}>
                ...
                />
                <Line type="monotone" dataKey={String(lastYear)} dot={false} opacity={0.45} strokeWidth={2} />
                <Line type="monotone" dataKey={String(currentYear)} dot={false} strokeWidth={3} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Section>

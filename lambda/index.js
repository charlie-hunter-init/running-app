...
async function loadJsonFromS3OrDefault(key, defaultValue) {
  try {
    ...
    console.log(`No readable S3 JSON found at s3://${BUCKET}/${key}. Continuing.`);
    return defaultValue;
  }
}
...
function buildStatsFromIndexItems(indexItems, timezone) {
  const weekly = {};
  const byShoe = {};
  const byYear = {};
  const byDayOfWeek = {};
...
exports.handler = async (event, context) => {
  ...
  if (mode === "incremental" && fetchedRuns.length === 0) {
    console.log("No new run activities found. Leaving existing S3 outputs unchanged.");
  ...
  const mergedStats = buildStatsFromIndexItems(mergedIndexItems, TIMEZONE);
